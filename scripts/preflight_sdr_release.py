...
def build_manifest(dist_dir: Path, lane: str, product_version: str) -> dict[str, object]:
    ...
    for path in sorted(item for item in dist_dir.rglob("*") if item.is_file()):
        ...
        # This Windows Qt distribution uses the operating-system ICU ABI.
        # A private Poppler/MSYS ICU with suffixed exports shadows Windows ICU
        # and causes ERROR_PROC_NOT_FOUND before QtWidgets can be imported.
        filename = path.name.casefold()
        if filename == "icuuc.dll" or (filename.startswith("icudt") and filename.endswith(".dll")):
            violations.append(f"{relative}: unapproved private ICU runtime")
...
def verify_manifest(dist_dir: Path, manifest_path: Path, lane: str, product_version: str) -> dict[str, object]:
    """Fail closed unless the stored manifest exactly describes the distribution."""

    try:
        stored = json.loads(manifest_path.read_text(encoding="utf-8-sig"))
    except (OSError, json.JSONDecodeError) as error:
        raise ValueError(f"cannot read release manifest: {manifest_path}") from error
    if not isinstance(stored, dict):
        raise ValueError("release manifest must be a JSON object")
    observed = build_manifest(dist_dir, lane, product_version)
    if stored != observed:
        raise ValueError("release manifest does not match distribution contents")
    return observed
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not isinstance(files, list):
        print("S12 preflight failed: release manifest files must be a list", file=sys.stderr)
        return 2
    print(
        json.dumps(
            {
