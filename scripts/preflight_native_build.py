"""Validate an explicitly staged native SDR module before Python tests."""
...
def main() -> int:
    ...
    if not module_path.is_file() or not manifest_path.is_file():
        raise SystemExit("native module or manifest is missing")
    ...
    import esw_dfl  # noqa: F401
    spec = importlib.util.spec_from_file_location("esw_dfl._sdr_native", module_path)
    if spec is None or spec.loader is None:
        raise SystemExit("cannot load staged native module")
    module = importlib.util.module_from_spec(spec)
    sys.modules["esw_dfl._sdr_native"] = module
    spec.loader.exec_module(module)
    info = dict(module.build_info())
    if bool(info.get("cuda_compiled")) != expected_cuda:
        raise SystemExit("build_info cuda_compiled does not match requested lane")
    if manifest.get("native_version") != info.get("version") or manifest.get("source_commit") is None:
        raise SystemExit("native build metadata does not match manifest")
