...
private:
    ...
    SampleLayout sample_layout_locked(const ReceiverSelection selection) const {
        ...
        return {
            ...
            .stride_bytes = buffer_ == nullptr
                ? static_cast<std::ptrdiff_t>(channels.size() * 2U)
                : api_->step(buffer_),
    ...
    void validate_selection_locked(const ReceiverSelection selection) const {
        ...
        if (selection == ReceiverSelection::Rx1 || selection == ReceiverSelection::Both) {
            require_pair(rx_i_, rx_q_, "RX1");
        }
        if (selection == ReceiverSelection::Rx2 || selection == ReceiverSelection::Both) {
            require_pair(rx2_i_, rx2_q_, "RX2");
        }
    }
    [[nodiscard]] std::vector<iio_channel*> selected_channels_locked(
        const ReceiverSelection selection
    ) const {
        validate_selection_locked(selection);
        switch (selection) {
    ...
    void disable_channels_locked(const std::vector<iio_channel*>& channels) noexcept {
        for (auto* channel : channels) {
            if (channel != nullptr && api_ != nullptr) api_->disable(channel);
        }
    }
