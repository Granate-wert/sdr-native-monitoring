...
class PersistenceWorkerTests(unittest.TestCase):
    ...
    def test_contiguous_short_rows_batch_exactly_with_full_visual_history(self):
        rng = np.random.default_rng(20260925)
        policy_revision = 7
        for value_mode in DensityValueMode:
            for mode in PersistenceRenderMode:
                for logarithmic in (False, True):
                    with self.subTest(value_mode=value_mode, mode=mode, logarithmic=logarithmic):
                        policy = PersistenceImagePolicy(policy_revision, mode, logarithmic)
                        history_reference = history_batched = None
                        for sequence in range(3):
                            values = rng.random((32, 4096), dtype=np.float32)
                            values[values < .96] = 0.0
                            values[::4] = -0.0  # Mixed zero and nonzero rows within one flat batch.
                            values[1, ::127] = np.nan
                            values[2, ::131] = np.inf
                            if value_mode is DensityValueMode.COUNT:
                                values *= 100.0
                            source = view(values, value_mode)
                            baseline_request = PersistenceImageRequest(source, policy, history_reference)
                            batched_request = replace(baseline_request, history=history_batched)
                            with patch.object(density_worker, "_flat_mapping_eligible", return_value=False):
                                reference = prepare_persistence_image(baseline_request)
                            batched = prepare_persistence_image(batched_request)
                            np.testing.assert_array_equal(
                                batched.image.view(np.uint32), reference.image.view(np.uint32))
                            self.assertEqual(batched.count_maximum, reference.count_maximum)
                            if mode is PersistenceRenderMode.VISUAL:
                                self.assertIsNotNone(batched.input_witness)
                                self.assertIsNotNone(reference.input_witness)
                                self.assertEqual(batched.input_witness.digest, reference.input_witness.digest)
                                self.assertIs(batched.input_witness.density(), source.density)
                            else:
                                self.assertIsNone(batched.input_witness)
                            self.assertFalse(batched.image.flags.writeable)
                            history_reference = reference.as_history(sequence + 1)
                            history_batched = batched.as_history(sequence + 1)

    def test_contiguous_flat_batch_reserves_scratch_and_cancels_after_one_chunk(self):
        source = view(np.full((32, 4096), .8, np.float32))
        policy = PersistenceImagePolicy(1, PersistenceRenderMode.VISUAL, False)
        history = prepare_persistence_image(PersistenceImageRequest(source, policy)).as_history(1)
        current = PersistenceImageRequest(source, policy, history)
        self.assertEqual(persistence_image_reserve(current),
                         source.density.nbytes + 65536 * 6)
        before = history.image.view(np.uint32).copy()
        cancelled = threading.Event()
        mapped_sizes = []
        original = density_worker.map_density_row_for_display

        def mapping(values, **kwargs):
            mapped_sizes.append(values.size)
        ...
        self.assertEqual(mapped_sizes, [65536])
        np.testing.assert_array_equal(history.image.view(np.uint32), before)

    def test_flat_mapping_crosses_row_boundaries_only_for_owned_c_layouts(self):
        eligible = density_worker._flat_mapping_eligible
        self.assertFalse(eligible(np.zeros((64, 1023), np.float32)))
        self.assertTrue(eligible(np.zeros((64, 1024), np.float32)))
        self.assertTrue(eligible(np.zeros((2, 65535), np.float32)))
        self.assertFalse(eligible(np.zeros((2, 65536), np.float32)))
        self.assertFalse(eligible(np.asfortranarray(np.zeros((32, 4096), np.float32))))
        self.assertFalse(eligible(np.zeros((32, 8192), np.float32)[:, ::2]))
        self.assertFalse(eligible(np.zeros((32, 4096), np.float32).view(np.ma.MaskedArray)))

        policy = PersistenceImagePolicy(2, PersistenceRenderMode.VISUAL, True)
        old = view(np.full((16, 5000), .25, np.float32))
        history = prepare_persistence_image(PersistenceImageRequest(old, policy)).as_history(1)
        values = np.random.default_rng(81).random((16, 5000), dtype=np.float32)
        values[::3] = -0.0
        values[1, ::127] = np.nan
        current = PersistenceImageRequest(view(values), policy, history)
        with patch.object(density_worker, "_flat_mapping_eligible", return_value=False):
